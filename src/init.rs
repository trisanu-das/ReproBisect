...
use crate::{
    model::RunnerBackend,
    output_discovery::{CandidateConfidence, discover_outputs},
};
...
pub fn create_config(
    ...
    let mut plan = detect_project(project);

    if probe_outputs {
        println!(
            "probing build outputs with {} in a temporary workspace...",
            runner.display_name()
        );
        match discover_outputs(project, &plan.image, &plan.command, runner) {
            Ok(result) => {
                ...
                if result.candidates.is_empty() {
                    ...
                    println!("possible final artifacts detected:");
                ...
                if result.scan_truncated {
                    plan.notes.push(
                        "output discovery hit its 100000-file scan bound; review the shortlist because additional candidates may exist"
                            .into(),
                    );
                }
            }
            Err(error) => {
                ...
                plan.notes.push(format!(
                    "temporary output discovery could not complete ({error:#}); kept static output inference"
                ));
            }
        }
    }

    fs::write(&path, render_config(&plan, runner))
    ...
    println!("runner: {}", runner.executable());
...
pub fn render_config(plan: &InitPlan, runner: RunnerBackend) -> String {
    ...
    out.push_str("\n[build]\n");
    out.push_str(&format!("runner = {}\n", string_value(runner.executable())));
...
mod tests {
    ...
    fn fallback_config_is_parseable() {
        let rendered = render_config(&generic_plan(), RunnerBackend::Docker);
    ...
    #[test]
    fn generated_config_can_select_podman() {
        let rendered = render_config(&generic_plan(), RunnerBackend::Podman);
        let parsed: Config = toml::from_str(&rendered).expect("parse generated config");

        assert_eq!(parsed.build.runner, RunnerBackend::Podman);
    }
}
